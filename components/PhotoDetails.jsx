...
const PhotoDetails = async ({ id, lang }) => {
    try {
        // console.log(id);
        // Fetch photo data
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/photos/${id}`);
        if (!response.ok) {
                    ...
                    <div className="mt-6">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button className="flex items-center gap-1.5 text-black/60 text-xs xl:text-sm">
                                <img src="/follow.svg" className="w-5 h-5" alt="follow icon" />
                                {dictionary.follow}
                            </button>
                        </div>
        ...
        return (
            <div>
                <p>Error: Something went wrong while fetching photo details.</p>
            </div>
        );
    }
};
